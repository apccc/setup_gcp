...
if [[ `$MY "SELECT id FROM ${SYSTEM_DATABASE}.cache WHERE key='"$KEY"' AND expires > NOW() LIMIT 1" | wc -l` -gt 0 ]];then
  echo 'fail'
else
  $MY "INSERT INTO ${SYSTEM_DATABASE}.cache (k,expires,data) VALUES('"$KEY"',ADDDATE(NOW(), INTERVAL "$DAYS" DAY),'"$($3 | base64)"')"
  echo 'ok'
fi
